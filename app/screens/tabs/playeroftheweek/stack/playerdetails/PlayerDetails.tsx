...
export default function PlayerDetails() {
  ...
  const { data, isFetching, error } = useQuery<PlayerPerformance>(
    "get player statistics",
    async () => {
      const result = await axios.get(
        `https://api.sofascore.com/api/v1/event/${root.params.eventId}/player/${root.params.playerId}/statistics`,
      );
      return result.data;
    },
  );
  ...
  if (error)
    return (
      <ErrorHandler message="Network Error. Please check your network and try again." />
    );
  ...
  return (
    <PlayerDetailsWrapper>
      ...
      {isFetching ? (
        <SkeltonPlayerDetailsLoader />
      ) : (
        <>
          <HeroSection style={{ elevation: 20 }}>
            <Avatar
              source={{
                uri: `https://api.sofascore.app/api/v1/player/${data?.player.id}/image`,
              }}
            ></Avatar>
            <Rating>
            ...
            <PlayerInfo>
              ...
              <Paragraph>
                {data?.player?.dateOfBirthTimestamp &&
                  new Date(
                    data.player.dateOfBirthTimestamp * 1000,
                  ).getFullYear()}
              </Paragraph>
              <Paragraph>
                {data?.player?.position === "G"
                  ? "GoalKeeper"
                  : data?.player?.position === "M"
                  ? "Midfielder"
                  : "Forward"}
              </Paragraph>
            </PlayerInfo>
