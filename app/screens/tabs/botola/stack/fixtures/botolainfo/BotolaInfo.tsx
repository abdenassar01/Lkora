...
export default function BotolaInfo() {
  const { data, isLoading, error } = useQuery<UniqueTournomant>(
    "get Botola Infos",
    async () => {
      const result = await axios.get(
        `https://api.sofascore.com/api/v1/unique-tournament/937`,
      );
    ...
    <>
      <BotolaInfoWrapper>
        <Card>
          <TeamPressable
            onPress={() =>
              navigation.navigate("Team", { id: data?.titleHolder.id })
            }
          >
            <Team>
              <TeamLogo
                source={{
                  uri: `https://api.sofascore.app/api/v1/team/${data?.titleHolder.id}/image`,
                }}
              ></TeamLogo>
        ...
        <Card>
          <TeamPressable
            onPress={() =>
              navigation.navigate("Team", { id: data?.mostTitlesTeams[0].id })
            }
          >
            <Team>
              <TeamLogo
                source={{
                  uri: `https://api.sofascore.app/api/v1/team/${data?.mostTitlesTeams[0].id}/image`,
                }}
              ></TeamLogo>
              <TeamName>{data?.titleHolder?.name}</TeamName>
            </Team>
