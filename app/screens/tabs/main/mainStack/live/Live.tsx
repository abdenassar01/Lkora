...
export default function Live() {
  ...
  if (error)
    return (
      <LiveError message="Network Error, Please chech your network status and try again" />
    );

  const filtred = data?.events?.filter(
    (event: Event) =>
      event.tournament.priority > 250 ||
      event?.tournament?.uniqueTournament?.id == 937,
  );

  return (
    <LiveSlider horizontal showsHorizontalScrollIndicator={false}>
      {filtred.length === 0 ? (
        <LiveError message="No live matches available at the moment check again later" />
      ) : (
        filtred.map((event: Event) => <LiveCard event={event} key={event.id} />)
      )}
    </LiveSlider>
  );
}
