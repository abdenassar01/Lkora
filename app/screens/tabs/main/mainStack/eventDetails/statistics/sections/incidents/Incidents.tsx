...
export default function Incidents({ id }: Props) {
  const { data, isFetching, error } = useQuery<Incident[]>(
    "match incidents",
    async () => {
  ...
  if (error)
    return (
      <MatchDetailSectionError message="incidents is not available for this event" />
    );
            ...
            <IncidentItem style={{ marginLeft: incident.isHome ? 0 : "40%" }}>
              {incident.incidentType === "substitution" ? (
                ...
                <IncidentParentElement>
                  <IncidentLogo
                    source={
                      incident.incidentType === "varDecision"
                        ? require("../../../../../../../../assets/var.png")
