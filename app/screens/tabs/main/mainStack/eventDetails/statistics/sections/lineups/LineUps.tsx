...
export default function LineUps({ id }: Props) {
  const { data, isFetching, error } = useQuery<LineUp>(
    "match stats",
    async () => {
  ...
  if (error || data?.confirmed === false)
    return (
      <MatchDetailSectionError message="LineUp is not available for this event" />
    );
